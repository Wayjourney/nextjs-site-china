...
export default function ProductPage({
  ...
  return (
    <main className='mx-auto max-w-screen-xl px-6 pb-24 pt-6 lg:px-8'>
      ...
      <div itemScope itemType="http://schema.org/Product" className='lg:grid lg:auto-rows-max lg:grid-cols-12 lg:gap-x-8'>
        <div className='mb-6 lg:col-span-5 lg:col-start-8 lg:mb-0'>
          <h1 itemProp="name" className='text-2xl font-bold text-gray-900'>{product?.name}</h1>
        </div>
            ...
            {product && (
              <div className='mt-4 text-base text-gray-500 space-y-4'>
                {product.description.map((item, index) => (
                  <p itemProp="description" key={index}>{item}</p>
                ))}
              </div>
            )}
          </div>
